...
$resourceView = Add-QueryResult -Label 'PostgreSQL Flexible Server ARM Resource' -Arguments @('resource', 'show', '--ids', $serverId)
...
$connectionString = Add-QueryResult -Label 'PostgreSQL Flexible Server Connection String' -Arguments @('postgres', 'flexible-server', 'show-connection-string', '--server-name', $ServerName, '--admin-user', (Get-SafePropertyValue -InputObject $server.Data -Path @('administratorLogin')), '--database-name', 'postgres')
...
$reportPath = Join-Path -Path $OutputDirectory -ChildPath ("PostgresqlFlexibleServerReport-{0}-{1}.md" -f $safeServerName, $timestamp)
...
Add-MarkdownLine -Builder $builder -Text ('# PostgreSQL Flexible Server Report: {0}' -f $ServerName)
Add-MarkdownLine -Builder $builder
...
Add-JsonSection -Builder $builder -Title 'PostgreSQL Flexible Server ARM Resource' -Result $resourceView
...
Add-JsonSection -Builder $builder -Title 'PostgreSQL Flexible Server Connection String' -Result $connectionString
