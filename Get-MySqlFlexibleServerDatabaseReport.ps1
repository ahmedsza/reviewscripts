...
Write-StatusMessage -Level 'INFO' -Message ('Starting MySQL Flexible Server database report for {0}/{1} in resource group {2}' -f $ServerName, $DatabaseName, $ResourceGroup)
...
$database = Add-QueryResult -Label 'MySQL Flexible Server Database Overview' -Arguments @('mysql', 'flexible-server', 'db', 'show', '--server-name', $ServerName, '--resource-group', $ResourceGroup, '--database-name', $DatabaseName) -Required

$serverId = $server.Data.id
$databaseId = $database.Data.id
$serverResourceView = Add-QueryResult -Label 'MySQL Flexible Server ARM Resource' -Arguments @('resource', 'show', '--ids', $serverId)
$databaseResourceView = Add-QueryResult -Label 'MySQL Flexible Server Database ARM Resource' -Arguments @('resource', 'show', '--ids', $databaseId)
...
$characterSetParameter = Add-QueryResult -Label 'MySQL Flexible Server Parameter: character_set_server' -Arguments @('mysql', 'flexible-server', 'parameter', 'show', '--server-name', $ServerName, '--resource-group', $ResourceGroup, '--name', 'character_set_server')
$collationParameter = Add-QueryResult -Label 'MySQL Flexible Server Parameter: collation_server' -Arguments @('mysql', 'flexible-server', 'parameter', 'show', '--server-name', $ServerName, '--resource-group', $ResourceGroup, '--name', 'collation_server')
$lowerCaseTableNamesParameter = Add-QueryResult -Label 'MySQL Flexible Server Parameter: lower_case_table_names' -Arguments @('mysql', 'flexible-server', 'parameter', 'show', '--server-name', $ServerName, '--resource-group', $ResourceGroup, '--name', 'lower_case_table_names')
...
$reportPath = Join-Path -Path $OutputDirectory -ChildPath ('MySqlFlexibleServerDatabaseReport-{0}-{1}-{2}.md' -f $safeServerName, $safeDatabaseName, $timestamp)
...
Add-MarkdownLine -Builder $builder -Text ('# MySQL Flexible Server Database Report: {0}/{1}' -f $ServerName, $DatabaseName)
Add-MarkdownLine -Builder $builder
...
Add-MarkdownLine -Builder $builder -Text 'This report uses Azure CLI management-plane commands. It captures Azure resource metadata for the server and the selected database, but it does not inspect tables, views, stored procedures, or row data inside MySQL.'
...
Add-JsonSection -Builder $builder -Title 'MySQL Flexible Server Database Overview' -Result $database
Add-JsonSection -Builder $builder -Title 'MySQL Flexible Server ARM Resource' -Result $serverResourceView
Add-JsonSection -Builder $builder -Title 'MySQL Flexible Server Database ARM Resource' -Result $databaseResourceView
...
Add-JsonSection -Builder $builder -Title 'MySQL Flexible Server Parameter: character_set_server' -Result $characterSetParameter
Add-JsonSection -Builder $builder -Title 'MySQL Flexible Server Parameter: collation_server' -Result $collationParameter
Add-JsonSection -Builder $builder -Title 'MySQL Flexible Server Parameter: lower_case_table_names' -Result $lowerCaseTableNamesParameter
